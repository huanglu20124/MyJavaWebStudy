...
<html>
  ...
  <script type="text/javascript">
  		function changePage(obj){
  		    var thispage = ${page.thispage};
  		    var countpage = ${page.countpage};
  			if(isNaN(obj.value)){
  				alert("必须是数字!");
  				obj.value=${page.thispage}
  				return;
  			}else if(obj.value > countpage){
  				alert("页码必须在有效范围内!");
  				obj.value= thispage;
  				return;
  			}else{
  				window.location.href="${pageContext.request.contextPath}/servlet/PageServlet?thispage="+obj.value;
  			}
  		}
  </script>
  </head>
  ...
  <body>
  	...
  	共${page.countrow }条记录
  	共${page.countpage }页
  	<a href="${pageContext.request.contextPath }/servlet/PageServlet?thispage=${page.firstpage }">首页</a>
  	<a href="${pageContext.request.contextPath }/servlet/PageServlet?thispage=${page.prepage }">上一页</a>
  	...
  	<c:forEach step="1" var="i" begin="${begin}" end="${end}">
  	    <c:if test="${i == page.thispage}">
  	        ${i}
  	    </c:if>
    	<c:if test="${i != page.thispage}">
  	        <a href="${pageContext.request.contextPath}/servlet/PageServlet?thispage=${i}">${i}</a>
  	    </c:if>	      	   
  	</c:forEach>
  	
  	  	<a href="${pageContext.request.contextPath }/servlet/PageServlet?thispage=${page.nextpage }">下一页</a>
  	<a href="${pageContext.request.contextPath }/servlet/PageServlet?thispage=${page.lastpage }">尾页</a>
  	
  	跳到<input type="text" value="${page.thispage}" style="width: 25px" onchange="changePage(this)">页
  </body>
</html>
